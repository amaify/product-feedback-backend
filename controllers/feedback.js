...
exports.newFeedback = async (req, res, next) => {
	try {
		...
		if (
			...
			return res
				.status(400)
				.json({ message: "Invalid input", statusCode: 400 });
		}
...
exports.getAllFeedback = async (req, res, next) => {
	try {
		...
		return res.status(401).json({ message: error.message, statusCode: 401 });
	}
};
...
exports.editFeedback = async (req, res, next) => {
	try {
		const user = await User.findById(req.userId);
		const editFeedbackId = await Feedback.findOne({
		...
		if (
			...
			return res
				.status(401)
				.json({ message: "Invalid input", statusCode: 401 });
		}
...
exports.deleteFeedback = async (req, res, next) => {
	try {
		if (!req.isAuth) {
			return res
				.status(400)
				.json({ message: "Not Authorized", statusCode: 400 });
		}
